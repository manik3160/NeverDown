             ...
             <div className="flex items-center gap-2.5 bg-[#fff7ef] border border-[#ffdbbd] px-4 py-2 mt-4 rounded-full shadow-sm">
               <div className="flex items-center gap-3">
                 ...
                 <span className={`text-[10px] uppercase font-extrabold tracking-[0.2em] ${
                    isCompleted ? 'text-green-500' : isFailed ? 'text-red-500' : 'text-[#ff6b00]'
                 }`}>
                   {isCompleted ? 'Completed' : isFailed ? 'Failed' : 'Active Analysis'}
                 </span>
                 {incident?.feedback_iteration > 1 && (
                    <span className="bg-orange-100 text-orange-600 px-2 py-0.5 rounded text-[9px] font-bold border border-orange-200">
                       REFINEMENT #{incident.feedback_iteration}
                    </span>
                 )}
               </div>
           </div>
             ...
             <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                ...
                <div className="p-8 pb-10 flex-1">
                    ...
                    <div className="flex items-center justify-between mt-10 mb-4">
                       <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Raw Execution Logs</div>
                       <div className="flex items-center gap-1.5">
                          <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></div>
                          <span className="text-[9px] font-bold text-gray-500 uppercase tracking-tight">Stream Live</span>
                       </div>
                    </div>
                    <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-xl p-4 font-mono text-[11px] text-gray-400 leading-relaxed shadow-2xl max-h-[400px] overflow-y-auto custom-scrollbar relative group">
                       <div className="absolute top-3 right-3 text-[9px] text-gray-600 font-bold uppercase opacity-0 group-hover:opacity-100 transition-opacity">
                          READ_ONLY_TRACE
                       </div>
                       {incident?.logs ? (
                          <pre className="whitespace-pre-wrap selection:bg-[#ff6b00]/30 selection:text-white">
                             {incident.logs}
                             <span className="inline-block w-1.5 h-4 bg-[#ff6b00]/40 ml-1 translate-y-0.5"></span>
                          </pre>
                       ) : (
                          <div className="flex items-center gap-2 italic text-gray-600">
                             <Activity className="w-3 h-3 animate-spin"/>
                             Awaiting detailed execution trace...
                          </div>
                       )}
                    </div>
                 </div>
             ...
             <div className="lg:col-span-1 space-y-6">
                ...
                <div className="bg-[#1c1c1c] rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
                   ...
                   <div className="flex items-center gap-2 mb-4">
                      <h3 className="text-[11px] font-extrabold text-[#ff6b00] uppercase tracking-[0.15em]">NeverDown Insight</h3>
